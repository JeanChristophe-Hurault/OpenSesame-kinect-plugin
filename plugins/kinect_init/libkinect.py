...
class libkinect:
    ...
    #"""Called by the kinect_init plugin - Initialize a Kinect object and a logfile."""
        ...
        # Initiate the counter of try and indicate that is the first recording
        self.first_enreg = True
        self.first_recording = True
        self.experiment.numEssai = 0
    ...
    #"""Get and record the data from the Kinect object."""
        ...
        while 1 :
            ...
            if self.recording == False :
                break
            tps = self.experiment.time() - t
            for skeleton in _kinect.skeleton_engine.get_next_frame().SkeletonData:
                print skeleton
                debug.msg(skeleton)
                if skeleton.eTrackingState == nui.SkeletonTrackingState.TRACKED:
                    # Indicate the correspondance of each articulations
